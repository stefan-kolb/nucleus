...
# TODO: download empty build log

shared_examples 'valid:logs:download' do
  describe 'download build log as .log', :as_cassette do
    before do
  ...
  describe 'download build log as .zip', :as_cassette do
    before do
      # TODO: at the time of writing this test, we assume that each platform provides a build log.
      # If this should not be the case, replace the build log with the first element in a queried log list
      get "/endpoints/#{@endpoint}/applications/paasal-test-app-all-updated/logs/build", request_headers
  ...
  describe 'download build log as .tar.gz', :as_cassette do
    before do
      # TODO: at the time of writing this test, we assume that each platform provides a build log.
      # If this should not be the case, replace the build log with the first element in a queried log list
      get "/endpoints/#{@endpoint}/applications/paasal-test-app-all-updated/logs/build", request_headers
  ...
  describe 'log download fails' do
    describe 'with non-existing application', :as_cassette do
      before do
        get "/endpoints/#{@endpoint}/applications/app_never_exists_0123456789/logs/build/download", request_headers
      end
      include_examples 'an unknown requested resource'
    end
    ...
    describe 'with non-existing log', :as_cassette do
      before do
        get "/endpoints/#{@endpoint}/applications/app_never_exists_0123456789/logs/build-xxxx/download", request_headers
      end
      include_examples 'an unknown requested resource'
    end
...
# shall be retrieved before application deployment so that the build log is actually empty
...
# shall be retrieved after application deployment so that the build log contains entries
shared_examples 'valid:logs:get' do
  ...
  describe 'get a log fails' do
    ...
    describe 'with non-existing application', :as_cassette do
      before { get "/endpoints/#{@endpoint}/applications/app_never_exists_0123456789/logs/build", request_headers }
      include_examples 'an unknown requested resource'
    end
  end
end
