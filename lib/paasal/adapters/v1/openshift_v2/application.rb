    ...
    module V1
      class OpenshiftV2 < Stub
        module Application
          ...
          def applications
            get('/applications').body[:data].collect { |application| to_paasal_app(application) }
          end

          # @see Stub#application
          def application(application_id)
            to_paasal_app(get("/application/#{app_id_by_name(application_id)}").body[:data])
          end
          ...
          def create_application(application_entity)
            ...
            to_paasal_app(updated_application[:data])
          end
          ...
          def to_paasal_app(app, gear_groups = nil, deployments = nil)
