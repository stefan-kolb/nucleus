    ...
    module V1
      class OpenshiftV2 < Stub
        ...
        module Services
          # @see Stub#services
          def services
            embedded_cartridges.collect { |cartridge| to_paasal_service(cartridge) }
          end

          # @see Stub#service
          def service(service_id)
            to_paasal_service(embedded_cartridge(service_id))
          end
          ...
          def installed_services(application_id)
            installed_cartridges(application_id).collect do |installed|
              to_paasal_installed_service(installed)
            end
          end
          ...
          def add_service(application_id, service_entity, plan_entity)
            ...
            to_paasal_installed_service(post("/application/#{app_id}/cartridges",
          ...
          def to_paasal_service(cartridge)
