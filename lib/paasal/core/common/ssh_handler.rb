module Nucleus
  class SSHHandler
    ...
    def create_private_key
      ...
      @key_file = File.expand_path(File.join(Dir.tmpdir, 'paasal', 'ssh', 'key', "#{SecureRandom.uuid}.pem"))
      # make sure key file path exists
      FileUtils.mkdir_p(File.dirname(@key_file))

      # generates default key of type RSA and with 2048 bits
      File.write(@key_file, SSHKey.generate(type: 'RSA', bits: 2048, comment: 'tmp_key_4_paasal').private_key)
      cache_public_key
    ...
    def create_agent
      ...
      @agent_file = File.expand_path(File.join(Dir.tmpdir, 'paasal', 'ssh', 'agent', SecureRandom.uuid))
    ...
    def cache_public_key
      @public_key = SSHKey.new(File.read(@key_file), comment: 'PaaSal').ssh_public_key
    rescue
