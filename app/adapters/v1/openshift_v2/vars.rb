      ...
      class OpenshiftV2 < Stub
        module Vars
          ...
          def env_vars(application_id)
            ...
            all_vars.collect { |var| to_paasal_var(var) }
          end
          ...
          def env_var(application_id, env_var_key)
            ...
            to_paasal_var(response.body[:data])
          end

          # @see Stub#create_env_var
          def create_env_var(application_id, env_var)
            to_paasal_var(post("/application/#{app_id_by_name(application_id)}/environment-variables",
          ...
          def update_env_var(application_id, env_var_key, env_var)
            to_paasal_var(put("/application/#{app_id_by_name(application_id)}/environment-variable/#{env_var_key}",
