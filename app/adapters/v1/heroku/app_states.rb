      ...
      class Heroku < Stub
        module AppStates
          def application_state(app, retrieved_dynos = nil)
            ...
            return API::Models::Application::States::DEPLOYED if dynos.empty?

            # 3: stopped if maintenance
            return API::Models::Application::States::STOPPED if app[:maintenance] || dynos_not_running?(dynos)

            # 4: running if no maintenance (checked above) and at least ony dyno is up
            return API::Models::Application::States::RUNNING if dyno_states(dynos).include?('up')

            # 5: idle if all dynos are idling
            return API::Models::Application::States::IDLE if dynos_idle?(dynos)
